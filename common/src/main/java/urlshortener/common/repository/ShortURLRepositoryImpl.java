...
public class ShortURLRepositoryImpl implements ShortURLRepository {
	...
	private static final RowMapper<ShortURL> rowMapper = new RowMapper<ShortURL>() {
		...
		public ShortURL mapRow(ResultSet rs, int rowNum) throws SQLException {
			return new ShortURL(rs.getString("hash"), rs.getString("target"),
					...
					rs.getString("country"), rs.getInt("clicks"));
		}
	};
	...
	public ShortURL save(ShortURL su) {
		try {
			jdbc.update("INSERT INTO shorturl VALUES (?,?,?,?,?,?,?,?,?,?)",
					...
					su.getIP(), su.getCountry(), su.getCount());
	...
	public void update(ShortURL su) {
		try {
			jdbc.update(
					"update shorturl set target=?, sponsor=?, created=?, owner=?, mode=?, safe=?, ip=?, country=?, clicks=? where hash=?",
					su.getTarget(), su.getSponsor(), su.getCreated(),
					su.getOwner(), su.getMode(), su.getSafe(), su.getIP(),
					su.getCountry(), su.getCount(), su.getHash());
		} catch (Exception e) {
	...
	@Override
	public void incCount(ShortURL su) {
		try {
			jdbc.update(
					"update shorturl set clicks = clicks + 1 where hash=?",
					su.getHash());
		} catch (Exception e) {
			log.debug("When update for hash " + su.getHash(), e);
		}
